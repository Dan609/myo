...
\title{Replicative aging of human mesenchymal stem cells is accompanied by changes in motility, cytoskeleton structure and nuclear-cytoplasmic RhoА localization}
\begin{document}
\date{today}
...
Quantitative image cytometry: For determining the relative nuclear DNA content of individual cells we used high-content Quantitative Image Cytometer CQ1 (Yokogawa) with Nipkow spinning disk confocal technology (Sakashita et al. 2015).

Initially cells were fixed in 4\% PFA for 30 min. After rinsing in PBS cells were permeabilized by 0.5\% Triton X-100 at room temperature for 10 minutes, rinsed again, plated on coverslips and stained with DAPI with final concentration 1.5 $\mu$g $mL^{−1}$.

Images were acquired with 405-nm laser and bright field illumination using 40x 0,95-NA dry objective lens. All images had a 2,560 × 2,160 pixel resolution, with a pixel size equivalent to 0.2 μm in x and y.

To calculate the DNA content of individual nuclei, the integrated intensity of the nuclear DAPI signal was measured in by automated image analysis using CQ1 software standard protocol. The histograms of DNA content in the cell population were then calculated to generate cell cycle profiles (using Grapher 13.0.629), and nuclei with the different relative nuclear DNA content were identified by applying visually selected cutoffs.
...
\begin{figure}
  \includegraphics[width=1\linewidth]{traj-corr.jpeg}
  \caption{Matrix of plots with a data set containing trajectory analysis parameters.}
  \label{traj-corr}
  \centering
\end{figure}
